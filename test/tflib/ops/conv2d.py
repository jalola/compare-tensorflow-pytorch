...
import pdb
DEBUG_MODE = 1
#DEBUG_CONST = 0.234234234
DEBUG_CONST = 5e-2

def init_fixed_data(shape):
    #n = 1
    #for i in range(len(shape)):
    #    n *= shape[i]
    #return np.linspace(0.0, 1.0, num=n).reshape(shape) 
    return np.full(shape, DEBUG_CONST)
...
def Conv2D(name, input_dim, output_dim, filter_size, inputs, he_init=True, mask_type=None, stride=1, weightnorm=None, biases=True, gain=1.):
    ...
    with tf.name_scope(name) as scope:
        ...
        filter_values *= gain
        # pdb.set_trace()
        # For testing purpose, using fixed data
        if DEBUG_MODE == 1:
            filter_values = init_fixed_data(filter_values.shape).astype('float32')
